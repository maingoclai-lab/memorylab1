export async function onRequest(context) {
  ...
  // Nếu bạn muốn "hardcode" key để không phải dùng Environment Variables (KHÔNG KHUYẾN KHÍCH VÌ BẢO MẬT):
  // Thay thế apiKey bằng: const apiKey = context.env.GEMINI_API_KEY || "AIzaSy...";
  
  if (!apiKey) {
    return new Response(JSON.stringify({ 
      error: "Chưa cấu hình GEMINI_API_KEY.",
      tip: "Bạn cần vào Dashboard Cloudflare Pages > Settings > Environment Variables và thêm GEMINI_API_KEY."
    }), { status: 500 });
  }
