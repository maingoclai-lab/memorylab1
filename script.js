...
function renderKanjiAnimations() {
    ...
    canvases.forEach(el => {
        ...
        const size = rect.width > 0 ? rect.width : 80;
        ...
        dictResult.innerHTML = `
            <div class="error-box">
                ...
                <div>
                    ...
                    <small>Đảm bảo bạn đã đặt GEMINI_API_KEY trong Environment Variables của Cloudflare Pages.</small>
                </div>
            </div>
        `;
        dictResult.style.display = "block";
...
window.removeTempCard = removeTempCard;
window.jumpToCard = (setName, idx) => {
    loadSet(setName);
    currentIndex = idx;
    updateCard();
